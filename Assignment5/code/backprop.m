...
mnistdisp(output);
drawnow;
title(['training set with err:' num2str(train_err)]);
...
for batch = 1:testnumbatches
 ...
 %%%% DISPLAY FIGURE TOP ROW REAL DATA BOTTOM ROW RECONSTRUCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%
fprintf(1,'Displaying in figure 2: Top row - testing data,Bottom row -- reconstructions \n');
output=[];
for ii=1:15
    output = [output data(ii,1:end-1)' dataout(ii,:)'];
end
...
mnistdisp(output);
drawnow
title(['Testing set with err:' num2str(test_err)] );
